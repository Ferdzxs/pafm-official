...
-- ============================================================================
-- EXTERNAL CITIZEN SYSTEM INTEGRATION (NO NEW TABLES)
-- Extend public.person for external citizen profile compatibility (Profile UI).
-- Idempotent: safe to run multiple times (IF NOT EXISTS).
-- ============================================================================

-- person: profile demographics + structured address (matches app PERSON_SELECT)
...
-- Legacy: older scripts used "Street_address"; folded to street_address in PG — no second column needed.

-- ============================================================================
-- PERSON: full_name and address MUST always have a non-empty value.
--
-- Four-step fix (idempotent — safe to re-run):
--   1. BACKFILL  — fix existing rows that are blank but have usable parts.
--   2. TRIGGER   — fires on ALL INSERT and ALL UPDATE (not just specific cols),
--                  always assigns a non-empty value via structured parts or a
--                  safe fallback so the column is never left blank.
--   3. CONSTRAINTS — CHECK constraints make it impossible to store '' via SQL.
-- ============================================================================

-- ─── STEP 1: Backfill existing rows ─────────────────────────────────────────
-- full_name: compute from name parts wherever the stored value is blank.
...
-- full_name: rows still blank after name-part pass → set a safe placeholder.
UPDATE public.person
SET full_name = 'Citizen'
WHERE full_name IS NULL OR btrim(full_name) = '';

-- address: compute from address parts wherever the stored value is blank.
UPDATE public.person
SET address = trim(concat_ws(', ',
    nullif(trim(coalesce(street_address, '')), ''),
    nullif(trim(coalesce(barangay,       '')), ''),
    nullif(trim(coalesce(city,           '')), ''),
    nullif(trim(coalesce(postal_code,    '')), ''),
    nullif(trim(coalesce(province,       '')), '')
  ))
...
-- address: rows still blank → set a safe placeholder.
UPDATE public.person
SET address = 'N/A'
WHERE address IS NULL OR btrim(coalesce(address, '')) = '';

-- ─── STEP 2: Trigger function ────────────────────────────────────────────────
-- Fires on EVERY insert/update so no path can bypass it.
-- Priority:
--   a) Structured name/address parts  → compute and overwrite.
--   b) Caller supplied a non-empty value  → keep it as-is.
--   c) Caller left it blank on UPDATE    → restore previous non-empty value.
--   d) Caller left it blank on INSERT    → assign a safe placeholder.
...
BEGIN
  ...
  IF v_full <> '' THEN
    -- (a) Structured parts present — trigger is the authority.
    NEW.full_name := v_full;
  ELSIF NEW.full_name IS NOT NULL AND btrim(NEW.full_name) <> '' THEN
    -- (b) Caller set a non-empty full_name explicitly — keep it.
    NULL;
  ELSIF TG_OP = 'UPDATE' THEN
    -- (c) UPDATE with blank full_name — restore the previous non-empty value.
    NEW.full_name := COALESCE(NULLIF(btrim(OLD.full_name), ''), 'Citizen');
  ELSE
    -- (d) INSERT with nothing usable — safe placeholder.
    NEW.full_name := 'Citizen';
  END IF;
  ...
  IF v_addr <> '' THEN
    -- (a) Structured parts present.
    NEW.address := v_addr;
  ELSIF NEW.address IS NOT NULL AND btrim(coalesce(NEW.address, '')) <> '' THEN
    -- (b) Caller set a non-empty address explicitly — keep it.
    NULL;
  ELSIF TG_OP = 'UPDATE' THEN
    -- (c) UPDATE with blank address — restore previous non-empty value.
    NEW.address := COALESCE(NULLIF(btrim(coalesce(OLD.address, '')), ''), 'N/A');
  ELSE
    -- (d) INSERT with nothing usable — safe placeholder.
    NEW.address := 'N/A';
  END IF;
...
-- ─── Recreate trigger (now fires on ALL INSERT / UPDATE) ─────────────────────
...
CREATE TRIGGER person_compute_full_name_address_trigger
  BEFORE INSERT OR UPDATE          -- every operation, not just specific columns
  ON public.person
  FOR EACH ROW
  EXECUTE PROCEDURE public.person_compute_full_name_and_address();

-- ─── STEP 3: CHECK constraints — DB-level enforcement ────────────────────────
-- Drop old constraints if they exist, then add fresh ones.
ALTER TABLE public.person
  DROP CONSTRAINT IF EXISTS person_full_name_nonempty;
ALTER TABLE public.person
  DROP CONSTRAINT IF EXISTS person_address_nonempty;

ALTER TABLE public.person
  ADD CONSTRAINT person_full_name_nonempty
    CHECK (full_name IS NOT NULL AND btrim(full_name) <> '');
ALTER TABLE public.person
  ADD CONSTRAINT person_address_nonempty
    CHECK (address IS NOT NULL AND btrim(coalesce(address, '')) <> '');

-- ============================================================================
-- Realtime (optional): enable Profile / inbox live updates in the app
-- Run in Supabase SQL Editor if you use postgres_changes subscriptions:
