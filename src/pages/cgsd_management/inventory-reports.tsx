        ...
        const { data, error } = await supabase
            ...
            .select(`
                *,
                ...
                inventory_request ( inventory_request_id, inventory_scope, status )
            `)
            ...
            <div className="flex flex-col md:flex-row md:items-center justify-between mb-8 gap-4">
                <div>
                    ...
                    <p className="text-muted-foreground text-sm mt-1">
                        View and download report documents for requesters; approvals are handled in the Approvals queue.
                    </p>
                </div>
                                ...
                                <tr key={item.inventory_report_id} className="hover:bg-accent/50 transition-colors">
                                    ...
                                    <td className="p-4 text-right flex flex-col items-end gap-1">
                                        <div className="flex gap-2">
                                            ...
                                            {item.digital_report_url && (
                                                <a
                                                    ...
                                                    download
                                                    className="inline-flex items-center justify-center gap-1.5 rounded-md border border-border px-2 py-1 text-xs font-medium text-foreground hover:bg-muted"
                                                >
                                        ...
                                        <div className="text-[10px] text-muted-foreground">
                                            Request: {item.inventory_request?.inventory_request_id || 'N/A'}
                                            {item.inventory_request?.status ? ` • ${item.inventory_request.status}` : ''}
                                        </div>
                                    </td>
                                </tr>
                            ))}
