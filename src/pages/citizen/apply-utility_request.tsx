...
const BUCKET_UTILITY_DOCS = "utility-docs";
...
export default function ApplyUtilityRequestPage() {
  ...
  const setCategory = (category: ServiceCategory) => {
    setForm((prev) => ({
      ...
      leak_subtype: category === "leak_report" ? "owner" : undefined,
    }));
  };
      ...
      for (const spec of uploadSpec) {
        ...
        for (let i = 0; i < files.length; i++) {
          ...
          if (upErr) {
            ...
            const docId = `URD-${Date.now()}-${spec.key}-${i}`;
            await supabase.from("utility_request_document").insert({
              id: docId,
              ticket_id: ticketId,
              requirement_key: spec.key,
              file_name: file.name,
              file_url: "", // upload failed; staff can request resubmission
            });
            continue;
          }
  ...
  return (
    <div className="px-4 py-6 sm:px-6 lg:px-8 animate-fade-in max-w-2xl mx-auto pb-12">
                ...
                <div className="flex gap-3">
                  {categoryConfig.subtypes.map((sub) => (
                    <button
                      key={sub.id}
                      type="button"
                      onClick={() => update("leak_subtype", sub.id)}
                      className={`px-3 py-2 rounded-lg text-sm border ${
              ...
              <section className="space-y-4">
                ...
                <div>
                  ...
                  <textarea
                    className="input-field w-full rounded-lg"
                    rows={3}
                    placeholder="Describe the drainage issue (declogging, desilting, repair...)"
...
export function UtilityRequirementChecklist({
  ...
  useEffect(() => {
    const load = async () => {
      const { data } = await supabase
        ...
        .select("id, ticket_id, requirement_key, file_name, file_url, uploaded_at, verified_at, verified_by")
  ...
  const expectedSpec = getRequirementUploadSpec(
    ticketType as ServiceCategory,
    (waterSubtype as WaterSubtype) ?? "no_existing_water",
    propertyType ?? "residential",
    (leakSubtype as LeakSubtype) ?? "owner"
  );
  ...
  return (
    <div className="mt-4 pt-4 border-t border-slate-700/50">
      <div className="text-xs font-semibold text-slate-400 uppercase tracking-wide mb-2">Requirements checklist — citizen uploads</div>
      <ul className="space-y-2">
        {docs.length === 0 && expectedSpec.length > 0 && (
          <li className="text-xs text-amber-400">No documents uploaded yet.</li>
        )}
        {docs.map((d) => (
          <li key={d.id} className="flex items-center gap-2 flex-wrap text-sm">
            <span className="text-slate-300">{keyToLabel[d.requirement_key] ?? d.requirement_key}:</span>
            <span className="text-slate-400 truncate max-w-[160px]" title={d.file_name}>
              {d.file_name}
            </span>
            {d.file_url ? (
              <a
              ...
              <span className="text-xs text-amber-400">(upload failed)</span>
            )}
            <label className="flex items-center gap-1.5 text-xs text-slate-400 ml-auto">
              <input
                type="checkbox"
                checked={!!d.verified_at}
                onChange={() => toggleVerified(d.id, !!d.verified_at)}
      ...
      {expectedSpec.length > 0 && docs.length > 0 && (
        <p className="text-[10px] text-slate-500 mt-2">
          Expected: {expectedSpec.map((s) => s.label).join("; ")}
        </p>
      )}
    </div>
  );
