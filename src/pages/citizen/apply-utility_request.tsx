...
export default function ApplyUtilityRequestPage() {
  ...
  return (
    <div className="px-4 py-4 sm:px-6 lg:px-8 animate-fade-in max-w-2xl mx-auto">
      <div className="mb-6">
        <h1 className="font-display text-2xl font-bold text-white">Utility Service Request</h1>
        <p className="text-slate-400 text-sm mt-0.5">Water Connection, Drainage & Leak Reports</p>
      </div>

      <div className="glass rounded-2xl p-6" style={{ border: '1px solid rgba(148,163,184,0.1)' }}>
        {/* Service Type Selection */}
        <div className="mb-6">
          <label className="block text-xs font-semibold text-slate-400 mb-2.5 uppercase tracking-wide">Select Request Type *</label>
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
            {[
              { id: 'water_connection', icon: Droplets, label: 'New Water Supply', desc: 'Apply for residential/commercial water connection' },
              { id: 'drainage', icon: MapPin, label: 'Drainage Request', desc: 'Request declogging or drainage repair' },
              { id: 'leak_report', icon: AlertTriangle, label: 'Report a Leak', desc: 'Report a public pipe leak or water obstruction' },
            ].map(type => {
              ...
              return (
                ...
                >
                  <Icon size={18} className={`mb-2 ${active ? 'text-blue-400' : 'text-slate-500'}`} />
                  <div className={`font-semibold text-sm mb-0.5 ${active ? 'text-blue-400' : 'text-slate-300'}`}>{type.label}</div>
        ...
        <form onSubmit={handleSubmit} className="space-y-4 animate-fade-in">
          <div>
            ...
            <input type="tel" className="input-field" placeholder="09xxxxxxxxx" value={form.contact_number} onChange={e => update('contact_number', e.target.value)} required />
          </div>

          <div>
            <label className="block text-xs font-semibold text-slate-400 mb-1.5 uppercase tracking-wide">
              {serviceType === 'leak_report' ? 'Exact Location of Leak *' : 'Service Address *'}
            </label>
            <textarea className="input-field" rows={2} placeholder="Complete address or landmark specifics..." value={form.address} onChange={e => update('address', e.target.value)} required />
          </div>

          {serviceType === 'water_connection' && (
            <div>
              <label className="block text-xs font-semibold text-slate-400 mb-1.5 uppercase tracking-wide">Property Type *</label>
              <select className="input-field" value={form.property_type} onChange={e => update('property_type', e.target.value)} required>
          ...
          {serviceType === 'leak_report' && (
            <div>
              <label className="block text-xs font-semibold text-slate-400 mb-1.5 uppercase tracking-wide">Severity Profile *</label>
              <select className="input-field" value={form.severity} onChange={e => update('severity', e.target.value)} required>
                <option value="low">Low (Minor drip or small puddle)</option>
                <option value="medium">Medium (Steady stream or pooling water)</option>
                <option value="high">High / Emergency (Geyser, flooding, or major road hazard)</option>
              </select>
            </div>
          )}
          ...
          <div className="px-4 py-3 mt-4 rounded-xl text-xs text-slate-400" style={{ background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(148,163,184,0.1)' }}>
            ...
            By submitting, your request will be immediately routed to the appropriate engineering dispatcher for validation.
          </div>
