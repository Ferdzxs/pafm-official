    ...
    <div className="px-4 py-4 sm:px-6 lg:px-8 max-w-6xl mx-auto animate-fade-in">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6">
        <div>
          ...
          <p className="text-slate-400 text-sm mt-0.5">Evaluate burial assistance requests for underprivileged families</p>
        </div>
      ...
      {loading ? (
        <div className="flex flex-col items-center justify-center py-20">
          ...
          <p className="text-slate-500 font-medium font-mono text-xs uppercase tracking-widest">Scanning Social Registry…</p>
        </div>
      ) : filtered.length === 0 ? (
        <div className="text-center py-20 glass rounded-2xl border border-slate-800">
          <ClipboardCheck className="mx-auto mb-4 text-slate-700" size={48} />
          <h3 className="text-lg font-semibold text-white">Queue is empty</h3>
          <p className="text-slate-500 max-w-xs mx-auto mt-2 text-sm">No cases match the current filter.</p>
        </div>
      ) : (
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {paginatedItems.map(app => {
            ...
            return (
              <Card key={app.application_id} className="card-hover bg-slate-900/40 border-slate-800/50 overflow-hidden">
                <CardContent className="p-0">
                  <div className="flex flex-col md:flex-row">
                    <div className="flex-1 p-6">
                      {/* App ID & Tags */}
                      <div className="flex flex-wrap items-center gap-2 mb-4">
                        <Badge variant="outline" className="text-[10px] font-mono border-slate-700 text-slate-400">{app.application_id}</Badge>
                      ...
                      <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
                        {/* Deceased */}
                        <div>
                          <p className="text-[10px] font-bold text-slate-500 uppercase tracking-widest mb-2">Deceased Individual</p>
                          <div className="flex items-center gap-3">
                            ...
                            <div>
                              ...
                              <p className="text-[10px] text-slate-500">DOD: {app.deceased?.date_of_death}</p>
                        ...
                        {/* Applicant */}
                        <div>
                          <p className="text-[10px] font-bold text-slate-500 uppercase tracking-widest mb-2">Relative / Applicant</p>
                          <div className="flex items-center gap-3">
                            ...
                            <div>
                              ...
                              <p className="text-[10px] text-slate-500">{app.person?.contact_number ?? '—'}</p>
                              <p className="text-[10px] font-mono text-slate-600">{app.person?.person_id}</p>
                            </div>
                          </div>
                        </div>
                      </div>
                    ...
                    {/* Action Panel */}
                    <div className="w-full md:w-64 bg-slate-900/60 border-t md:border-t-0 md:border-l border-slate-800/50 p-6 flex flex-col justify-between gap-4">
                      <div>
                        <p className="text-[10px] font-bold text-slate-500 uppercase tracking-widest mb-2">Submitted</p>
                        ...
                        {iar?.social_worker_eval_date && (
                          <div className="mt-2">
                            <p className="text-[10px] font-bold text-slate-500 uppercase tracking-widest mb-1">Eval Date</p>
                      ...
                      {isProcessed ? (
                        <div className="p-3 rounded-xl bg-white/5 border border-slate-800 text-center">
                          {app.application_status === 'ssdd_evaluated' ? (
        ...
        >
          ...
          >
            <div className="flex items-center justify-between p-6 border-b border-white/5 bg-white/3">
              <div>
                <h2 className="text-lg font-bold text-white">Indigent Assistance Review</h2>
            ...
            <div className="p-6 space-y-3">
              {[
                ...
                <div key={label} className="flex justify-between py-2 border-b border-white/5">
                  ...
                  <span className="text-sm text-white text-right max-w-[55%]">{value}</span>
                </div>
              ))}
            </div>
            <div className="p-6 pt-0 flex gap-2">
              ...
              <button
                ...
                className="flex-1 py-2.5 rounded-xl bg-emerald-500/10 text-emerald-400 border border-emerald-500/20 hover:bg-emerald-500/20 transition-all text-xs font-bold uppercase shadow-inner"
              >
