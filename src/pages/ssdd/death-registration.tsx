...
export default function DeathRegistrationInfoForSSDD() {
 ...
 return (
  ...
  {stages.map((s, i) => (
   ...
   <div className={`flex flex-col items-center ${i > 0 ? '' : ''}`}>
    ...
    <span className={`text-[9px] mt-1 font-bold ${current >= s.step ? 'text-blue-400' : 'text-muted-foreground'}`}>{s.label}</span>
   </div>
   {i < stages.length - 1 && (
    <div className={`h-px w-8 mb-3 mx-1 transition-colors ${current > s.step ? 'bg-blue-500' : 'bg-muted'}`} />
   )}
   </React.Fragment>
  ))}
  </div>
 ...
 <div className="px-4 py-4 sm:px-6 lg:px-8 animate-fade-in">
  ...
  <div>
   ...
   <p className="text-muted-foreground text-sm mt-0.5">Track indigent burial cases through the Death Registration Division workflow</p>
  </div>
  <div className="relative">
   ...
   className="input-field pl-10 text-sm w-full sm:w-72"
   value={searchTerm}
  ...
  <div className="text-center py-20 glass rounded-2xl border border-border">
   <Activity className="mx-auto mb-4 text-muted-foreground" size={40} />
   <p className="text-muted-foreground">No cases to coordinate.</p>
  </div>
  ) : (
  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
   {paginatedItems.map(r => {
   const stage = STAGE_MAP[r.application_status] ?? { label: r.application_status, color: 'text-muted-foreground' }
   const iar = r.indigent_assistance_record?.[0]
   return (
    <Card key={r.application_id} className="bg-card/40 border-border/50 card-hover">
    <CardContent className="p-0">
     ...
     <div className="flex-1 p-6">
      ...
      <div>
       ...
       <span className="text-[10px] font-mono text-muted-foreground">{r.application_id}</span>
      ...
      <div><span className="text-muted-foreground">Applicant: </span>{r.person?.full_name}</div>
      <div><span className="text-muted-foreground">DOD: </span>{r.deceased?.date_of_death}</div>
      <div><span className="text-muted-foreground">Submitted: </span>{r.submission_date}</div>
      {iar && <div><span className="text-muted-foreground">IAR: </span><span className="text-emerald-400">{iar.assistance_id}</span></div>}
      </div>
     </div>

     <div className="w-full md:w-48 bg-card/60 border-t md:border-t-0 md:border-l border-border/50 p-5 flex flex-col items-center justify-center gap-2">
      {r.deceased?.death_certificate_no && (
      <div className="text-center mb-2">
       <p className="text-[10px] text-muted-foreground uppercase font-bold mb-0.5">Cert. #</p>
    ...
    {selected && (
     <div className="card-premium mx-auto w-full animate-in zoom-in-95 duration-300 flex flex-col max-h-[90vh]">
      <DialogHeader className="mb-6 space-y-1 text-left shrink-0">
       <DialogTitle className="font-display text-2xl font-extrabold tracking-tight text-foreground leading-tight flex items-center justify-between">
        <span>{selected.deceased?.full_name}</span>
       </DialogTitle>
       <DialogDescription className="font-mono text-xs text-muted-foreground mt-1 tracking-widest uppercase">
      ...
      <div className="space-y-4 overflow-y-auto flex-1 sidebar-scrollbar pb-2">
       ...
       <div className="mt-4 p-4 border border-border/50 bg-background rounded-xl shadow-sm">
