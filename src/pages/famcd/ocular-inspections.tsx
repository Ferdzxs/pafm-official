...
export default function OcularInspectionsPage() {
 ...
 const handleSubmitReport = async () => {
  ...
  const employeeId = user.employee_id || 'EMP-004' // Default placeholder if employee record not found
  ...
  const { error: inspError } = await supabase
   ...
   .insert({
    ...
    conducted_by_office: user.role === 'famcd' ? 'OFF-004' : (user.office || null),
  ...
  if (uploadedEvidenceUrls.length > 0) {
   const evidenceRecords = uploadedEvidenceUrls.map(url => ({
    evidence_id: `EVID-${Date.now()}-${Math.random().toString(36).substring(2, 11)}`,
    inspection_id: inspectionId,
    photo_url: url,
  ...
  const reportId = `IRP-${Date.now()}-${Math.random().toString(36).substring(2, 7)}`
  const { error: repError } = await supabase
   ...
   .insert({
    ...
    prepared_by_office: user.role === 'famcd' ? 'OFF-004' : (user.office || null),
