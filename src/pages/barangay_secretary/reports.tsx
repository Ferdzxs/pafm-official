...
export default function BarangaySecretaryReports() {
 ...
 try {
  ...
  setStats({
   ...
   pendingReservations: formattedReservations.filter((r) =>
   ["submitted", "pending", "awaiting_treasury", "order_of_payment_issued", "payment_pending"].includes(r.status),
   ).length,
   approvedReservations: formattedReservations.filter((r) =>
   ["permit_issued", "completed", "confirmed", "approved"].includes(r.status),
   ).length,
   forwardedToPB: formattedReservations.filter((r) =>
   ["pending_pb_approval", "pb_approved", "forwarded", "forwarded_to_pb"].includes(r.status),
   ).length,
