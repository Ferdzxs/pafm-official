...
import { TrendingUp, TrendingDown, Clock, CheckCircle, Users, Package, FileText, Layout, AlertCircle, ArrowUpRight, Loader2 } from 'lucide-react'
import {
    AreaChart,
    Area,
    XAxis,
            ...
            <div className="mb-8 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                <div>
                    <div className="flex items-center gap-2 mb-2">
                        <span className="px-2.5 py-1 rounded-md text-xs font-semibold" style={{ background: `${meta.bgColor}22`, color: meta.color }}>
                            {meta.label}
                        </span>
                    </div>
                ...
                <button 
                  ...
                  className="gradient-primary px-5 py-2.5 rounded-xl text-sm font-bold text-white shadow-lg shadow-blue-500/20 hover:scale-[1.02] transition-all flex items-center gap-2 self-start sm:self-auto"
                >
                ...
                {kpis.map((kpi, i) => (
                    <Card key={i} className="glass border-white/5 overflow-hidden">
                        <CardContent className="pt-5 p-4">
                            ...
                            <div className="text-xs text-slate-400 mt-1 uppercase tracking-tight">{kpi.label}</div>
                        </CardContent>
                    </Card>
                ))}
            </div>
                ...
                <Card className="lg:col-span-2 glass border-white/5">
                    ...
                    <CardContent className="p-0">
                        {loading ? (
                            ...
                            <div className="py-20 text-center text-slate-500">No recent activity found.</div>
                        ) : (
                            <div className="divide-y divide-white/5">
                                {recentApplications.map(app => (
                                    <div key={app.application_id} className="flex items-center gap-3 p-4 hover:bg-white/5 transition-colors cursor-pointer">
                                        <div className="w-8 h-8 rounded-lg bg-blue-500/10 flex items-center justify-center text-blue-400 text-xs font-bold shrink-0">BA</div>
                                        <div className="flex-1 min-w-0">
                                            <div className="text-sm font-medium text-white truncate">New Burial Application: {app.deceased?.full_name}</div>
                                            <div className="text-xs text-slate-400 truncate">Submitted by {app.person?.full_name}</div>
                                        </div>
                                        <div className="flex items-center gap-3 shrink-0">
                                            <Badge className={`text-[10px] px-2 py-0.5 font-bold uppercase ${
                                                app.application_status === 'pending' ? 'bg-amber-500/10 text-amber-500 border-amber-500/20' : 'bg-emerald-500/10 text-emerald-500 border-emerald-500/20'
                                            }`}>
                                                {app.application_status}
                                            </Badge>
                                            <span className="text-[10px] text-slate-500 font-mono">{new Date(app.submission_date).toLocaleDateString()}</span>
                                        </div>
                                    </div>
                                ))}
                            </div>
                ...
                <div className="space-y-6">
                    <Card className="glass border-white/5">
                        <CardHeader className="pb-2">
                            <CardTitle className="text-sm font-bold uppercase tracking-wider text-slate-400">Niche Distribution</CardTitle>
                        </CardHeader>
                        <CardContent>
                            ...
                            <div className="flex justify-center gap-4 mt-2">
                                <div className="flex items-center gap-1.5"><div className="w-2 h-2 rounded-full bg-emerald-400" /><span className="text-[10px] text-slate-400">Available</span></div>
                                <div className="flex items-center gap-1.5"><div className="w-2 h-2 rounded-full bg-red-400" /><span className="text-[10px] text-slate-400">Occupied</span></div>
                            </div>
                        </CardContent>
                    </Card>
                    ...
                    <Card className="glass border-white/5 p-4">
                        ...
                        <div className="space-y-2">
                            {[
                                ...
                                <a key={link.label} href={link.path} className="flex items-center gap-3 p-3 rounded-xl bg-white/5 hover:bg-white/10 transition-all border border-white/5">
