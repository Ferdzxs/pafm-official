     ...
     <div className="card-premium mx-auto w-full animate-in zoom-in-95 duration-300 max-h-[90vh] flex flex-col sidebar-scrollbar">
      <DialogHeader className="mb-6 space-y-1 text-left shrink-0">
       <div className="flex items-center gap-3">
        ...
        <div>
         <DialogTitle className="font-display text-xl font-extrabold tracking-tight">New Asset Request</DialogTitle>
         <DialogDescription className="font-medium text-muted-foreground/80 mt-0.5">Submitting to FAMCD — Cemetery Office</DialogDescription>
        </div>
       </div>
      </DialogHeader>

      <form onSubmit={handleSubmit} className="space-y-4 overflow-y-auto flex-1 sidebar-scrollbar pb-2">
       <div>
        <label className="block text-[10px] font-bold text-muted-foreground uppercase tracking-widest mb-1.5 ml-1">Item / Service Requested *</label>
        <input
       ...
       <div>
        <label className="block text-[10px] font-bold text-muted-foreground uppercase tracking-widest mb-1.5 ml-1">Priority</label>
        <select
         className="w-full px-3 py-2 rounded-xl text-sm border bg-background border-border outline-none focus:ring-2 focus:ring-primary/50 text-foreground"
         value={formPriority}
       ...
       <div>
        <label className="block text-[10px] font-bold text-muted-foreground uppercase tracking-widest mb-1.5 ml-1">Notes (optional)</label>
        <textarea
         className="w-full px-3 py-2 rounded-xl text-sm border bg-background border-border outline-none focus:ring-2 focus:ring-primary/50 text-foreground resize-none"
       ...
       <div className="pt-8 flex flex-col-reverse sm:flex-row sm:items-center sm:justify-end gap-3 shrink-0">
        <button type="button" onClick={() => setShowModal(false)} className="h-11 rounded-xl px-8 w-full sm:w-auto border border-border bg-background text-[11px] font-extrabold uppercase tracking-widest text-foreground hover:bg-muted transition-all">Cancel</button>
        <button type="submit" disabled={submitting}
         className="h-11 rounded-xl flex-1 sm:flex-none px-6 text-[11px] font-extrabold shadow-lg shadow-blue-500/20 uppercase tracking-widest bg-blue-600 hover:bg-blue-700 text-white gap-2 disabled:opacity-50 flex items-center justify-center transition-all">
         <Send size={15} className="mr-1.5" /> {submitting ? 'Submitting...' : 'Submit to FAMCD'}
        </button>
       </div>
      </form>
     </div>
