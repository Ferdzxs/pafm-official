...
import {
    Sun, Moon, Bell, Settings, Search, ChevronRight,
    User, LogOut, Menu, X
} from 'lucide-react'
...
export function TopBar({ sidebarCollapsed, onToggleSidebar }: TopBarProps) {
    ...
    return (
        <header
            className="flex-shrink-0 flex items-center gap-3 px-3 sm:px-5"
            style={{
                height: '60px',
        ...
        >
            ...
            <button
                ...
                className="flex-shrink-0 w-9 h-9 rounded-lg flex items-center justify-center transition-all md:w-8 md:h-8"
            ...
            <div className="flex-1 flex justify-center">
                <div
                    className="relative w-full max-w-xs sm:max-w-md lg:max-w-[520px]"
                >
                    <Search
                        size={14}
            ...
            <div className="flex items-center gap-1 flex-shrink-0">
                    ...
                    {showNotifs && (
                        ...
                        >
                            ...
                            >
                                ...
                                <span className="text-xs text-blue-500">{unreadCount} unread</span>
                            </div>
                            <div className="overflow-y-auto" style={{ maxHeight: '340px' }}>
                                {notifications.length === 0 ? (
                                    ...
                                    <div
                                        ...
                                        onMouseEnter={e => ((e.currentTarget as HTMLElement).style.background = 'var(--color-popup-item-hover)')}
                                        onMouseLeave={e => ((e.currentTarget as HTMLElement).style.background = n.is_read ? 'transparent' : 'rgba(59,130,246,0.05)')}
                                    >
                                        <div className="flex items-start gap-2">
                                            <div
                                                className="w-1.5 h-1.5 rounded-full mt-1.5 flex-shrink-0"
                                                style={{ background: n.is_read ? 'transparent' : '#3b82f6' }}
                                            />
                                            <div>
                                                <div className="text-xs font-semibold" style={{ color: 'var(--color-text-primary)' }}>{n.title}</div>
                                                <div className="text-xs mt-0.5 leading-relaxed" style={{ color: 'var(--color-popup-subtext)' }}>{n.message}</div>
                ...
                {/* Divider */}
                <div className="w-px h-5 mx-1" style={{ background: 'var(--color-sidebar-border)' }} />
                ...
                <div className="relative" ref={userRef}>
                    ...
                    >
                        ...
                        <div className="hidden sm:block text-left">
                            <div className="text-xs font-semibold leading-tight" style={{ color: 'var(--color-sidebar-user)' }}>
                                {user?.full_name.split(' ').slice(-1)[0]}
                            </div>
                            <div className="text-[10px] leading-tight" style={{ color: 'var(--color-sidebar-email)' }}>
                                {user?.role.replace(/_/g, ' ')}
                            </div>
                        </div>
                        <ChevronRight size={12} className={`hidden sm:block transition-transform ${showUser ? 'rotate-90' : ''}`} style={{ color: 'var(--color-sidebar-sub)' }} />
                    </button>
                    ...
                    {showUser && (
                        ...
                        >
                            {/* User info header */}
                            <div className="px-4 py-3" style={{ borderBottom: '1px solid var(--color-sidebar-border)' }}>
                                <div className="text-xs font-semibold" style={{ color: 'var(--color-text-primary)' }}>{user?.full_name}</div>
                                <div className="text-[10px] mt-0.5 truncate" style={{ color: 'var(--color-text-muted)' }}>{user?.email}</div>
                            </div>
                            <NavLink
                                to="/profile"
...
export default function Sidebar({ collapsed }: SidebarProps) {
    ...
    return (
        <aside
            className={clsx(
                'fixed inset-y-0 left-0 z-40 flex flex-col sidebar-glow transition-transform duration-300 select-none flex-shrink-0 md:static md:translate-x-0',
                collapsed
                    ? '-translate-x-full md:w-[68px]'
                    : 'translate-x-0 w-64 md:w-[220px]'
            )}
            style={{
                background: 'var(--color-sidebar-bg)',
        ...
        >
            ...
            >
                ...
                {!collapsed && (
                    <div className="mt-3 text-center animate-fade-in">
                        <div className="font-display font-bold text-sm leading-snug" style={{ color: 'var(--color-sidebar-title)' }}>
                            Public Assets
                        </div>
                        <div className="text-xs leading-snug" style={{ color: 'var(--color-sidebar-sub)' }}>
                            &amp; Facilities Mgmt
                        </div>
                    </div>
                )}
            ...
            <nav className="flex-1 overflow-y-auto px-2 mt-3 space-y-0.5 pb-24 md:pb-4">
                {navItems.map(item => {
                    ...
                    const badge = item.badgeKey ? BADGE_COUNTS[item.badgeKey] : 0
                    return (
                        <NavLink
                            key={item.id}
                            to={item.path}
                        ...
                        >
                            ...
                            {!collapsed && (
                                <>
                                    <span className="flex-1 truncate animate-fade-in text-[13px]">{item.label}</span>
                                    {badge > 0 && (
                                        <span
